...
func TestMatch(t *testing.T) {
	...
	j = []byte(LargeJson2)
	m = []string{
		`entity.datastreams.#(label=="content")#.file_format|@iany:["MS Word Document","MS Excel Spreadsheet","MS PowerPoint Presentation","MS Visio Diagram","Office Open XML Document","Office Open XML Presentation","Office Open XML Spreadsheet","Text","datastreams"]`,
		`action|@ne:"enriched"`,
	}
	if !Matches(j, m) {
		t.Errorf("%s | %s", j, m)
	...
	// check usage of iany
	j = []byte(`{"entity":{"datastreams":[{"label":"content", "file_format": "content-type-1"}]}}`)
	m = []string{"entity.datastreams.#(label==\"content\")#.file_format|@any:[\"\\\"content-type-1\\\"\", \"\\\"content-type-2\\\"\"]"}
	if !Matches(j, m) {
		t.Errorf("%s | %s", j, m)
	}
